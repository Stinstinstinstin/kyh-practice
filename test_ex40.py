...
# check_integer_relation()
def test_check_integer_relation():
    assert check_integer_relation(2, 1, 3) is True


def test_check_integer_relation_wrong_params():
    assert check_integer_relation(2, 3, 1) is False


def test_check_integer_relation_same_value():
    assert check_integer_relation(1, 1, 1) is False


def test_check_integer_relation_same_value_not_true():
    assert check_integer_relation(1, 1, 1) is not True


def test_check_integer_relation_same_values_zero():
    assert check_integer_relation(0, 0, 0) is False


def test_check_integer_relation_negative_value():
    assert check_integer_relation(-10, -20, 0) is True
